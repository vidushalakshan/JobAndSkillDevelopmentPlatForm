...
export const Button = ({
  ...
  const baseStyles =
    "relative overflow-hidden rounded-full font-bold leading-tight flex items-center justify-center gap-2 focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 transition-colors duration-50";

  const sizeStyles = {
    small: "px-4.5 py-3.5 text-[10px] text-[16px]",
    medium: "px-6 py-4 text-[17px]", 
  ...
  const variantStyles = {
    ...
    bgBlack: "bg-[#2f302f] text-white hover:text-white border border-white/10",
  };

  const hoverBgAnimation =
    variant === "primary"
      ? "before:absolute before:top-0 before:left-0 before:h-full before:w-0 hover:before:w-full before:bg-blue-700 before:z-0 before:transition-all before:duration-500 before:ease-in-out before:shadow-[0_4px_12px_rgba(33,137,255,0.3)] hover:text-white"
      : variant === "bgBlack"
